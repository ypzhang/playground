...
namespace gs {
  ...
  public:
    virtual bool read_matrix(const char *filename) = 0;

    int32_t num_rows() const { return m_num_rows; }
    int32_t num_cols() const { return m_num_cols; }
    int32_t num_nnzs() const { return m_num_nnzs; }

    const int *get_row_ptrs() const {  return m_row_ptrs.data(); }
    const int *get_rows() const {  return m_rows.data(); }
    const int32_t *get_cols() const {  return m_cols.data(); }
  ...
  public:
    virtual bool read_matrix(const char *filename);
  private:
  };
